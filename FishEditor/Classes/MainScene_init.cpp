...
// 获取声音文件的播放时长
float GetSoundSeconds(std::string const& soundFileName) {
	float rtv = 0.0f;
	//std::thread t([&] {
	//	auto&& id = cocos2d::experimental::AudioEngine::play2d(soundFileName);
	//	do {
	//		std::this_thread::sleep_for(std::chrono::milliseconds(1));
	//		rtv = cocos2d::experimental::AudioEngine::getDuration(id);
	//	} while (rtv == -1);
	...
	};
	{
		...
		for (auto&& item : filesIterator) {
			...
			case FishManage::FileExtensions::wav: {
				...
				file = o;
				//o->seconds = GetSoundSeconds(name);	// 晚些时候填
